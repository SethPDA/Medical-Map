...
public class JsonArrayAdapterMapPharmacy extends ArrayAdapter<Pharmacy> {
	private final Context context;
	private final ArrayList<Pharmacy> values;
	private ArrayList<Pharmacy> filteredValues;
	private Filter filter;

	public JsonArrayAdapterMapPharmacy(Context context,
			...
			if (prefix != null && prefix.toString().length() > 0) {
				...
				for (int i = 0; i < tmpItems.size(); i++) {
					...
					if (sf.Name.toUpperCase().contains(prefix)
							|| sf.Phone.toUpperCase().contains(prefix)) {
